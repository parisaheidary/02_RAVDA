function [J,gradJ]=Reduced_Adjoint_fun_param(Par0,Par_b,POD,M)
...
S0=(param_T(1)*z'.^2+param_T(2)*(-z')+param_T(3));%+0.03;
...
lper=length(t_d)+1;
% B0=diag([(1/1)*ones(1,1),(1/0.01)*ones(1,1)]); %B0=err^2
B0=diag([(1/1)*ones(1,1),(1/1)*ones(1,1),(1/1)*ones(1,1),(1/1)*ones(1,1),(1/0.01)*ones(1,1)]); %B0=err^2
% B0=1/0.9;
R=1/625;%(0.04)^2;%1/(0.04)^2;%1/0.4;%1/(0.04)^2; %R=err^2
%%%% calibration coefficient for adjusting SHPs in gradient calculation-
...
hold off
% ylim([-1 1])
% set(gcf,'Position',[50 450 800 300])
set(gcf,'Position',[2050 450 800 300])
title('nu')
