% this code includes the optimization loop.
clear all;
close all;clc; 
...
% load('C:\Users\parisaheidary\Box\Mywork_Recharge\RichardsCodes\theta93.mat');
% St0=flip(theta93');
teta_rT=0.0883;
teta_sT=0.4662;
alphaT=0.0085;
mT=2.5795;  %Ksat_T=13.19;
xT=0.4101;  %nT=1.507;
...
%% Perturbing obs
a=[-0.04,0.04];
% r = a(1) + 2*a(2)*rand(1,90); %% in range a
r=(normrnd(0,0.04,[1,90]));
% r=0;
% observ=truth(1,:);
...
num_par=5;
teta_r0=0.008; %0.0883;
teta_s0=0.389;  %0.4662;
alpha0=0.023;%;
x0=log(1.23);     %  n=exp(x)
m0=log(2.7183);   %  K=exp(m)   
S_0=St0;
% S_0=St0(1,1)*ones(lz,1);

%% OL
% to have S_mean
%%% without considering error of obs and forcing
[t0,S00]=Hydrus_Run_param(S_0,teta_r0,teta_s0,alpha0,x0,m0);
%%% considering err of obs and forcing
...
%%% reading output fluxes- w/o errs
% [t_q_ol,vtop_ol,q_bot_ol,inf_ol,Ev_ol]=reading_flx_Hy;
%%% reading outut fluxes- w/ errs
[t_q_ol,vtop_ol,q_bot_ol,inf_ol,Ev_ol]=reading_flx_Hy_err;



S0=St0;
% [POD,M_,M_k,M]=P_M_computing_param(S0,alpha0,K0);

%%% I created POD and M once and loaded here in order to save time
...
load(fn,'POD_comb','M_','M')



options=optimset('Display','iter','MaxIter',1000,'TolFun',0.5, 'MaxFunEvals', 100,'TolX', 0.005, 'LargeScale','off','GradObj','on','DerivativeCheck','off','HessUpdate','bfgs','FinDiffType','forward');%,'LineSearchType','quadcubic');%,'TolGradCon', 1e-6, 'TolCon', 1e-6);%,'Diagnostics','on');%,'PlotFcns',@myplotfun1);'CheckGradients','on'  %%'HessUpdate','dfp' 'TolGradCon', 1e-10

Par0=[teta_r0;teta_s0;alpha0;x0;m0]; 
Par_b(:,1)=Par0;

n=0; J_i(1)=100;miu=100;
while miu>1e-25
        
    n=n+1
    
    [Par0_up,fval,exitflag,output,grad,hessian]=fminunc(@(Par0)Reduced_Adjoint_fun_param(Par0,Par_b(:,n),POD_comb,M),Par0,options);
    
    J_i(n+1)=fval;
    miu(n+1)=abs((J_i(n+1)-J_i(n)))/max(abs(J_i(n+1)),1);
...
[t_q_est,vtop_est,q_bot_est,inf_est,Ev_est]=reading_flx_Hy;
% [t_q_est,vtop_est,q_bot_est,inf_est,Ev_est]=reading_flx_Hy_err;
...
cc_T=ismember(t_q_T,t_d);
i_qT=find(cc_T);

cc_e=ismember(t_q_est,t_d);
i_qe=find(cc_e);

cc_ol=ismember(t_q_ol,t_d);
i_qo=find(cc_ol);
...
RMSE_qo=sqrt(mean((q_bot_T(i_qT)-q_bot_ol(i_qo)).^2))
bias_qo1=mean(q_bot_T(i_qT)-q_bot_ol(i_qo))
cor_qo=corrcoef(q_bot_T(i_qT),q_bot_ol(i_qo))

RMSE_qe=sqrt(mean((q_bot_T(i_qT)-q_bot_est(i_qe)).^2))
bias_qe=mean(q_bot_T(i_qT)-q_bot_est(i_qe))
cor_q=corrcoef(q_bot_T(i_qT),q_bot_est(i_qe))


figure;
plot(t_q_T,vtop_T,'g','LineWidth',2)
hold on
plot(t_q_ol,vtop_ol,'b','LineWidth',1.5)
hold on
plot(t_q_est,vtop_est,'k--','LineWidth',2)
legend({'True','OL','RA-VDA'},'fontweight','bold','fontsize',18,'Orientation','horizontal')
ylabel('actual surface flux [cm/day]')
xlabel('Day')
% ylim([0.05 0.4])
set(gcf,'position',[100,200,1110,500])
set(gca,'FontName','Times New Roman','FontSize',20,'LineWidth', 1.2);
...
RMSE_vtop_o=sqrt(mean((vtop_T(i_qT)-vtop_ol(i_qo)).^2))
cor_vtop_o=corrcoef(vtop_T(i_qT),vtop_ol(i_qo))
bias_vtop_o=mean(vtop_ol(i_qo)-vtop_T(i_qT))

RMSE_vtop_e=sqrt(mean((vtop_T(i_qT)-vtop_est(i_qe)).^2))
cor_vtop_e=corrcoef(vtop_T(i_qT),vtop_est(i_qe))
bias_vtop_e=mean(vtop_est(i_qe)-vtop_T(i_qT))


figure;
plot(t_q_T,Ev_T,'g','LineWidth',2)
hold on
plot(t_q_ol,Ev_ol,'b','LineWidth',1.5)
hold on
plot(t_q_est,Ev_est,'k--','LineWidth',2)
legend({'True','OL','RA-VDA'},'fontweight','bold','fontsize',18,'Orientation','horizontal')
ylabel('actual cumulative Evap [cm/day]')
xlabel('Day')
% ylim([0.05 0.4])
set(gcf,'position',[100,200,1110,500])
set(gca,'FontName','Times New Roman','FontSize',20,'LineWidth', 1.2);
...
figure;
plot(t_q_T,inf_T,'g','LineWidth',2)
hold on
plot(t_q_ol,inf_ol,'b','LineWidth',1.5)
hold on
plot(t_q_est,inf_est,'k--','LineWidth',2)
legend({'True','OL','RA-VDA'},'fontweight','bold','fontsize',18,'Orientation','horizontal')
ylabel('actual cumulative inf [cm/day]')
xlabel('Day')
% ylim([0.05 0.4])
set(gcf,'position',[100,200,1110,500])
set(gca,'FontName','Times New Roman','FontSize',20,'LineWidth', 1.2);
...
RMSE_Ev_o=sqrt(mean((Ev_T(i_qT)-Ev_ol(i_qo)).^2))
bias_Ev_o=mean(Ev_ol(i_qo)-Ev_T(i_qT))

RMSE_Ev_e=sqrt(mean((Ev_T(i_qT)-Ev_est(i_qe)).^2))
bias_Ev_e=mean(Ev_est(i_qe)-Ev_T(i_qT))

RMSE_inf_o=sqrt(mean((inf_T(i_qT)-inf_ol(i_qo)).^2))
bias_inf_o=mean(inf_ol(i_qo)-inf_T(i_qT))

RMSE_inf_e=sqrt(mean((inf_T(i_qT)-inf_est(i_qe)).^2))
bias_inf_e=mean(inf_est(i_qe)-inf_T(i_qT))
...
RMSE_ol_end=sqrt(mean((theta0(end,:)-truth(end,:)).^2))
RMSE_es_end=sqrt(mean((S_est(end,:)-truth(end,:)).^2))
